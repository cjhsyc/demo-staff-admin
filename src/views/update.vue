    ...
    <el-form label-width="100px" :model="staff" :rules="rules" status-icon ref="updateForm">
      ...
      <el-form :inline="true" label-width="100px" :model="staff" :rules="rules" status-icon ref="inlineForm2">
        <el-form-item label="入职日期" prop="entryDate">
          <el-date-picker v-model="staff.entryDate" type="date" value-format="yyyy-MM-dd"></el-date-picker>
        </el-form-item>
      ...
      <el-form-item label="个人简介">
        <el-upload class="upload-demo" drag action="mock/introduction" :limit="2" multiple
                   :before-upload="beforeUpload" :on-exceed="exceed" :file-list="staff.introduction"
                   :on-success="uploadSuccess" :on-remove="removeFile">
...
export default class update extends Vue {
  ...
  countyList: KeyValue[] = []
  rules: Object = rules
  allowFileTypes: string[] = ['application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/pdf']
  ...
  uploadSuccess(response: Result) {
    this.staff.introduction.push(response.data)
  }
  ...
  async saveUpdate() {
    ...
    this.staff.address = getAddress(this.provinceCode, this.cityCode, this.countyCode)
    const result = await api.saveStaffs(this.staff)
    if (result.code === 200) {
      this.$message({message: result.data.message, type: result.data.type})
      if (result.data.type === 'success') {
        await this.$router.push({name: 'query', params: {needUpdate: 'true'}})
      }
    }
  }
  ...
  mounted() {
    if (this.$route.params.staff) {
      ...
      const {provinceCode, cityCode, countyCode} = getAddressCode(this.staff.address)
...
.el-form--inline .el-form-item .el-input, .el-select, .el-input-number {
  width: 20vw;
}
